...
class SoundSystem {
  ...
  playWordAudio(wordId, onEnded = null) {
    ...
    // 處理空白檔名如 "wake up" -> "V7_wake%20up.mp3"
    const fileName = `V7_${wordId}.mp3`;
    const audioPath = `V7_flashcards_audios/${fileName}`;
    ...
    audio.onerror = () => {
      // 嘗試備援父目錄路徑
      const backupPath = `../V7_flashcards_audios/${fileName}`;
  ...
  playZhAudio(wordId, onEnded = null) {
    ...
    const fileName = `V7_${wordId}_zh.mp3`;
    const audioPath = `V7_flashcards_audios/${fileName}`;
    const audio = new Audio(encodeURI(audioPath));
    this.currentVoice = audio;
